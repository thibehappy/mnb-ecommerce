...
## 14. Champs front-end ajoutés depuis cette doc

Ces 4 champs ont été ajoutés au front sur la branche `claude/extensions`
APRÈS la rédaction des sections précédentes. Ils ne cassent rien (tous
optionnels), mais le back doit les conserver dans `BraceletConfig` /
`BraceletComponent` pour que l'artisan voie les choix utilisateur.

### `BraceletComponent.flipped?: boolean`
Drapeau d'orientation : ajoute +180° à la rotation visuelle de la perle.
N'a d'effet que sur les formes asymétriques (cœur, étoile, nœud, fleur)
où le PNG catalogue a une orientation fixe et que l'utilisateur veut
retourner (ex. cœur pointe en bas → en haut). Préservé dans le snapshot
panier + URLs partagées. **Le back peut l'ignorer pour la logique métier**
(prix, stock, expédition) — c'est purement visuel — mais doit le
transmettre à l'artisan dans la fiche d'assemblage.

### `Charm.extraFee?: number`  (Euros)
Supplément facturé **uniquement** quand un charm dépasse le quota inclus
dans l'atelier (`atelier.maxCharms`, ex. Classique = 3 charms inclus).
Tier prix : 1 € (entrée de gamme), 3 € (premium). Fallback :
`DEFAULT_EXTRA_CHARM_FEE = 1 €` si le champ est absent.

⚠ **À distinguer de `Charm.surcharge`** : `surcharge` s'applique à chaque
pose (licence Sanrio/Disney = 6 €) ; `extraFee` ne s'applique qu'aux
charms en surplus. Un même charm peut combiner les deux (rare).

**Calcul prix bracelet (autoritatif côté back) :**
```
base = atelier.price
+ Σ charm.surcharge          // pour chaque charm posé (licences)
+ extraCharmsFee(components) // somme des extraFee pour les charms
                             // au-delà de atelier.maxCharms (cord-order)
+ figurine.surcharge         // licence Kawaii Sanrio/Disney
```

### `BraceletConfig.figurineChainId?` + `figurineClaspId?`
Système d'attache figurine ↔ bracelet pour l'atelier Kawaii. Deux
catalogues séparés (à exposer via `/api/attachments/chains` et
`/api/attachments/clasps` une fois côté back). Modèles :

```ts
type ChainColor = 'black' | 'orange' | 'red';
interface AttachmentChain {
  id: string;        // 'attach_chain_red'
  name: string;
  color: ChainColor;
  hex: string;
  images: string[];
...
interface AttachmentClasp {
  id: string;        // 'attach_clasp_heart_blue'
  name: string;
  shape: ClaspShape;
  hex: string;
  dotsHex?: string;
...
Stockage front : `public/photos/attachments/<id>.png`. À synchroniser
quand le back livrera son endpoint catalogue.

**Ces 2 champs sont snapshottés sur `BraceletConfig` UNIQUEMENT quand
l'atelier est `atelier_kawaii` ET qu'une figurine est sélectionnée.**
Sinon ils restent `undefined` dans le payload commande — le back ne doit
PAS s'attendre à les voir sur du Classique / Bracelet Bar.

**Catalogue actuel :**
- 3 chaînes : `attach_chain_red`, `attach_chain_orange`, `attach_chain_black`
- 4 anneaux : `attach_clasp_round_yellow`, `attach_clasp_round_green`,
  `attach_clasp_heart_blue`, `attach_clasp_heart_orange`

Aucun surcoût : ces accessoires sont inclus dans le prix de l'atelier
Kawaii (24 €). Le back doit juste les conserver dans la commande pour
